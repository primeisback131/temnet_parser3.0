...
public class SessionAdminController {
     ...
     */
    @GetMapping
    public List<ActiveSession> list() {
        List<ActiveSession> rows = new ArrayList<>();
        Set<String> seenUsers = new HashSet<>();
        Set<String> seenIps = new HashSet<>();
        for (ActiveSessions.Entry e : activeSessions.list()) {
            rows.add(new ActiveSession(e.username, e.ip, e.userAgent, e.loginAt, e.lastSeen,
                    loginAttempts.isBlocked(e.username, e.ip)));
        ...
        for (LoginAttemptService.Lockout lockout : loginAttempts.blockedUsers()) {
            if (seenUsers.add(lockout.username())) {
                rows.add(new ActiveSession(lockout.username(), lockout.ip(), "", null, null, true));
                seenIps.add(lockout.ip());
            }
        }
        for (String ip : loginAttempts.blockedIps()) {
            if (seenIps.add(ip)) {
                rows.add(new ActiveSession("", ip, "", null, null, true));
            }
        }
        return rows;
    }
