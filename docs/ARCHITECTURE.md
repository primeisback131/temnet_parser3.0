...
- **Оператор** — аккаунт поддержки; `username` начинается с `help`
  (`help`, `helpm`, `help-dnk`, …). В новых метриках (operators, categories)
  используется префикс `LIKE 'help%'`; в SLA/chat исторически —
  подстрока `LIKE '%help%'` (см. примечание в [METRICS.md](METRICS.md)).
...
- **Даты** на границе API — `LocalDate` (ISO `yyyy-MM-dd`); внутри запросов
  ...
  [METRICS.md](METRICS.md).
- **Идентификатор оператора** — `help%`; есть исторический нюанс с `%help%`
  в части запросов (описан в METRICS.md).
